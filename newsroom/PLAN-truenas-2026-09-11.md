...
1. **英文/一手源没有中文标题** —— 早报第 7–10 条是 OpenAI News / Simon Willison 的原始英文标题，
   模板轨不做翻译（零创作契约）。要中文就得靠生成层，或先在 `sources.yaml` 里降权/过滤。
...
阶段 5 的硬承诺是 07:30 / 19:00 准点，但现在挂上去只会得到**每天准时产出一份质量未定稿的早报**。
两个前置条件没满足：

1. 上面的「已知缺口 1」（英文条目标题）不解决，早报的观感就达不到上线标准；
2. `emit.sh` 的 push 环节需要 D29 的细粒度 token，否则每天的提交都会**堆在 NAS 本地**
   —— 不是错误，但会让"NAS 是否真的发布了"变得不可观测。

结论：**先补缺口 1，再挂 cron**。挂的时候用 `midclt cronjob`（D30），不要裸放 systemd 单元。
...
7. **决定英文/一手源条目怎么处理** —— 早报里 OpenAI News、Simon Willison 这类条目
   目前是原样英文标题（模板轨零创作，不翻译）。三个选项：
   ① 生成层做中文标题（要花钱、要等阶段 4）；② `sources.yaml` 里给纯英文源降权或过滤；
   ③ 接受中英混排（内容更全，观感更杂）。**这是产品决策，不是技术决策。**
