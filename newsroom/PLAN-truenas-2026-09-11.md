...
| 3.7 | 生产线挂 cron | ⏸ | **刻意未挂** —— 见下方「为什么不马上挂 cron」 |
...
阶段 5 的硬承诺是 07:30 / 19:00 准点。前置条件已从两条收敛到**一条（+一条待你拍板）**：

1. ~~英文/一手源没有中文标题~~ → **F9 已解决**（接入 aihot 后，早报 4/10 条为中文标题 + 中文摘要）。
2. **D29 细粒度 token 仍未到位** → `emit.sh` 能 commit 不能 push，每天提交会**堆在 NAS 本地**
   —— 不是错误，但会让"NAS 是否真的发布了"变得不可观测。**唯一硬前置。**
3. **需要你拍板**：早报可以使用 AIHOT 的**改写摘要**吗？RUNBOOK 原红线写的是
   "禁止照抄 aihot 摘要成文"。本轮把它按产品形态拆开了 ——
   **索引型（早报/日报，清单）**可以（必须署名 `AIHOT·<出处>` + 页脚声明），
   **成稿型（洞察，生成层）**禁止。这个拆分需要你确认，我不替你决定合规边界。

结论：**拿到 token + 确认第 3 条，即可挂 cron**。挂的时候用 `midclt cronjob`（D30），不要裸放 systemd 单元。
...
> 更新于 2026-09-11 20:55。前 2 项是当前唯一的硬阻塞。
> 现状：**生产线本身已端到端跑通**（阶段 1–3 全绿），下面这些人工动作是它能否"无人值守跑起来"的前置。
...
7. ~~**决定英文/一手源条目怎么处理**~~ → **已用第 4 种方案解决**：接入 aihot 线索层，
   由它提供中文标题与摘要（署名 `AIHOT·<出处>`、原文链接保留、页脚声明）。
   代价是引入了外部改写，所以**需要你确认合规边界**（见下面第 8 条）。
8. **确认 AIHOT 摘要在早报里的使用边界** —— 现按产品形态拆分：
